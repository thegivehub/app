...
class DonateButton extends HTMLElement {
    ...
    async connectedCallback() {
        // Fetch supported cryptocurrencies via donation API
        try {
            const response = await fetch("/donation-api.php?action=crypto&supported=1");
            const json = await response.json();
            this.supportedCryptos = json.data || {};
        } catch (error) {
        ...
        if (window.tghAnalytics && typeof window.tghAnalytics.track==='function') {
            window.tghAnalytics.track('donate_view', { component: 'DonateButton' });
        }
    }
    ...
    render() {
        const styles = `
            ...
            .presets button { padding:8px 12px; border:1px solid var(--border-color); background:#f8f9fa; cursor:pointer; border-radius:4px; }
            .recurring { display:flex; align-items:center; gap:8px; }
                ...
                <div class="modal-content">
                    ...
                    <form id="donateForm">
                        <div>
                            ...
                            <button type="button" class="mobile-pay-btn" id="mobilePayBtn">Pay with Mobile (Payment Request)</button>
                        </div>
                        ...
                        <div class="recurring">
                          ...
                          <select id="frequency" name="frequency" style="margin-left:auto;">
    ...
    setupEventListeners() {
        ...
        const presetBtns = this.shadowRoot.querySelectorAll('.presets button');
        const mobilePayBtn = this.shadowRoot.querySelector('#mobilePayBtn');
        ...
        donateBtn.addEventListener("click", () => {
            ...
            // Show mobile payment button if Payment Request API is available and method is card
            if (window.PaymentRequest) {
                mobilePayBtn.style.display = 'inline-block';
            } else {
        ...
        presetBtns.forEach(b=> b.addEventListener('click', ()=>{
            const amt = parseFloat(b.getAttribute('data-amt'));
            this.shadowRoot.querySelector('#amount').value = amt.toFixed(2);
        }));
        ...
        form.addEventListener("submit", async (e) => {
            ...
            try {
                if (this.paymentMethod === "crypto") {
                    ...
                    const response = await fetch("/donation-api.php?action=crypto", {
                        method: "POST",
                    ...
                    // Handle card payment via process endpoint (supports recurring)
                    const payload = {
                        amount: data.amount,
                        currency: 'USD',
                        campaignId: data.campaignId,
                        paymentMethod: 'credit_card',
                        donorInfo: { name: data.name, email: data.email },
                        recurring: this.shadowRoot.querySelector('#recurring').checked,
                        frequency: this.shadowRoot.querySelector('#frequency').value
                    };
                    const response = await fetch("/donation-api.php?action=process", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                    ...
                    if (result.success) {
                        ...
                        form.reset();
                        if (window.tghAnalytics) window.tghAnalytics.track('donate_success', { method: 'card', recurring: payload.recurring });
                    } else {
                        throw new Error(result.error);
                    }
                }
                ...
                if (window.tghAnalytics) window.tghAnalytics.track('donate_fail', { method: this.paymentMethod, error: error.message });
            }
        });
    }
