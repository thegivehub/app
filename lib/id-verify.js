  ...
  async function showVerificationReport(verificationId) {
    try {
      ...
      const showDate = new Intl.DateTimeFormat("en-US").format(new Date());
        ...
        <div class="report-section">
          ...
          <div class="status-grid">
            ...
            <div class="status-item">
              ...
              <span class="status-value">${showDate}</span>
            </div>
          </div>
        </div>
      `;
      ...
      if (documentImageUrl) {
        ...
        idImageUrl = `/uploads/document/${verificationId}_drivers_license.jpg?_t=${timestamp}`;
      }
          ...
          <div class="document-image">
            <img src="${idImageUrl}" alt="ID Document" 
                 onerror="if(this.src.indexOf('.png') > -1) { this.src = this.src.replace('.png', '.jpg'); } 
                         else if(this.src.indexOf('/api.php/') > -1) { this.src = '/uploads/document/${verificationId}_drivers_license.jpg'; }
      ...
      if (selfieImageUrl) {
        ...
        // Direct filename based on verification ID
        selfieImageSrc = `/uploads/selfie/${verificationId}_selfie.png?_t=${timestamp}`;
      }
        ...
        <div class="document-preview">
          ...
          <div class="document-image">
            <img src="${selfieImageSrc}" alt="Selfie"
                 onerror="if(this.src.indexOf('.png') > -1) { this.src = this.src.replace('.png', '.jpg'); } 
                         else if(this.src.indexOf('/api.php/') > -1) { this.src = '/uploads/selfie/${verificationId}_selfie.png'; }
  ...
  document.getElementById('verification-form').addEventListener('submit', async function(e) {
    ...
    try {
      ...
      let apiUrl = '/api.php/verifications';
      let apiMethod = 'POST';
  ...
  async function uploadIdDocument() {
    ...
    try {
      ...
      try {
        ...
        response = await fetch('/api.php/documents/upload', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
          },
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('id-upload-btn').addEventListener('click', async function() {
      ...
      try {
        ...
        // Upload document
        const response = await fetch('/api.php/documents/upload', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
          },
  ...
  async function checkExistingVerification() {
    try {
      ...
      const response = await fetch('/api.php/verifications/status/status-for-user', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`
        }
