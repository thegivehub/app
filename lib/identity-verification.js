    ...
    const verificationModule = {
        ...
        async submitVerification() {
            try {
                ...
                formData.append('documentFile', this.data.documentFile);
                formData.append('documentType', this.data.documentType);
                formData.append('selfieFile', this.data.selfieFile);

                // Upload document first
                const documentUploadResponse = await fetch('/api/document/upload', {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
                    },
                ...
                if (!documentResult.success) {
                    throw new Error(documentResult.error || 'Failed to upload documents');
                }
