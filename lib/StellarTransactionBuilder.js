...
class StellarTransactionBuilder {
  ...
  async buildTransaction(sourceSecret, operationBuilder, options = {}) {
    try {
      ...
      // Determine the per-operation base fee. Callers can opt into network-aware
      // fee optimization via options.optimizeFee; otherwise we keep the existing
      // congestion-based recommendation so existing callers are unaffected.
      let baseFee;
      if (options.optimizeFee) {
        // getOptimizedFee(1, ...) returns the per-operation fee; the SDK
        // TransactionBuilder multiplies it by the number of operations.
        const optimizePriority = ({
          low: 'low',
          medium: 'normal',
          high: 'high'
        })[options.feePriority || 'medium'] || 'normal';
        baseFee = await this.feeManager.getOptimizedFee(1, optimizePriority);
      } else {
        // Get recommended fee based on priority
        baseFee = await this.feeManager.getRecommendedFee({
