...
import StellarFeeManager from 'StellarFeeManager';
...
class StellarTransactionBuilder {
  constructor(config = {}) {
    ...
    // Fee settings (now managed by feeManager)
   ...
   * @param {string} priority - Transaction priority ('low', 'medium', 'high')
   * @returns {Promise<number>} - Fee in stroops
   */
  async getDynamicFee(priority = 'medium') {
    try {
      return await this.feeManager.getRecommendedFee({ priorityLevel: priority });
    } catch (error) {
      this.log('Error getting dynamic fee', error);
      return this.baseFee;
    }
  }
  ...
  async createPayment({
    ...
    try {
      ...
      // Get dynamic fee based on priority
      const fee = await this.getDynamicFee(feePriority);
      this.log(`Using dynamic fee: ${fee} stroops (priority: ${feePriority})`);
  ...
  async submitTransaction(transaction, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        this.log(`Submitting transaction, attempt ${attempt}/${maxRetries}`);
        const result = await this.server.submitTransaction(currentTx);
        ...
        lastError = error;
        this.log(`Error submitting transaction (attempt ${attempt}/${maxRetries})`, error);
        ...
        const isFeeError = this.feeManager.isFeeRelatedError(error);
        
        if (isFeeError && attempt < maxRetries && options.sourceSecret) {
          this.log('Fee-related error detected, will retry with higher fee');
          
          // Create fee bump transaction for the next attempt
          try {
            currentTx = await this.feeManager.createFeeBumpTransaction(
              options.sourceSecret,
              currentTx
            );
            this.log('Created fee bump transaction for retry');
          } catch (bumpError) {
            this.log('Failed to create fee bump transaction', bumpError);
            // Continue with original transaction if fee bump fails
          }
        ...
        // For non-fee errors or if we're out of retries, break out of the loop
        ...
        const delay = Math.min(retryDelay * Math.pow(2, attempt - 1) + Math.random() * 1000, 30000);
        this.log(`Will retry in ${delay}ms`);
    ...
    return {
      ...
      errorDetail: (lastError?.response?.data?.extras?.result_codes)
        ? JSON.stringify(lastError.response.data.extras.result_codes)
        : 'Unknown error'
    };
  }
